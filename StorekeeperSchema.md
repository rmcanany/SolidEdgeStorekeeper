...
Change the name and enter the relevant information.
...
Since I *want* things like that, we have developed a workaround.  For illegal characters, we made "stand-ins" that are translated by Storekeeper when building the tree view of the library.  (The tree view is a Control in WinForms, not the underlying XML itself.  It is not picky about such things.)
...
- You might be wondering why we haven't had to worry about all this before.  That is because when the program parses the Excel files, it does all the replacements in the process.  Since this new method *starts* with XML, we have to deal with it.  (Except the companion spreadsheets, which remain in Excel format.)
